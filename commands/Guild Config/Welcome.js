...
module.exports = {
  ...
  run: async (client, message, args) => {
    if (!args[0])
      return message.reply({allowedMentions: {
    ...
    if (args[0].toLowerCase() === "set-channel") {
      ...
      message.reply({allowedMentions: {    
        parse: ['everyone', 'users', 'roles'],    
        repliedUser: false,
        }, embed: client.embed({ description: `Channel set to <#${channel.id}>`}, message)})
    }
    ...
    if (args[0].toLowerCase() === "set-message") {
      ...
      if (!msg) return message.reply({allowedMentions: {    
        parse: ['everyone', 'users', 'roles'],    
        repliedUser: false,
        }, embed: client.embed({ description: `Provide a welcome message.\nDon't forget you can use {guild} to use the guild name.\nYou can also use {user} to mention the user that is joining.`,}, message)})
      await WelcomeConfig.findOneAndUpdate(
        { guildId: message.guild.id },
        { message: msg }
      );
      message.reply({allowedMentions: {    
        parse: ['everyone', 'users', 'roles'],    
        repliedUser: false,
        }, embed: client.embed({ description: `Welcome message set to ${msg}.`}, message)})
    }
    if (args[0].toLowerCase() === "enable") {
      ...
      message.reply({allowedMentions: {    
        parse: ['everyone', 'users', 'roles'],    
        repliedUser: false,
        }, embed: client.embed({ description: `Welcome system enabled.`}, message)})
    }
    if (args[0].toLowerCase() === "disable") {
      ...
      message.reply({allowedMentions: {    
        parse: ['everyone', 'users', 'roles'],    
        repliedUser: false,
        }, embed: client.embed({ description: `Welcome system disabled.`}, message)})
    }
  },
};
