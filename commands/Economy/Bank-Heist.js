...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        if(!user)  return message.reply({ embed: client.embed({ description: `Mention a user.`}, message)})
        ...
        if(userToken < 5000) return message.reply({ embed: client.embed({ description: `The person you are trying to rob doesn't have at least 5000 Shiba Token in their bank.`}, message)})
        ...
        if(chance < 75) {
            ...
            message.reply({ embed: client.embed({ description: `You have been caught trying to rob <@${user.id}>. You had to pay them ${tokenCharge} Shiba Token`}, message)})
            ...
            message.reply({ embed: client.embed({ description: `You have successfully robbed <@${user.id}>'s bank! Your payout is ${payout} Shiba Token.`}, message)})
