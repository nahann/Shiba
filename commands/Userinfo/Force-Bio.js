...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        if(data) await UserinfoConfig.findOneAndUpdate({ userId: member.id }, { Bio: bio })
        if(!data) await UserinfoConfig.create({
            userId: member.id,
            Bio: bio,
        })
        message.lineReplyNoMention(client.embed({ description: `${member.username}'s bio set to: \`${bio}\``}, message))
    }
}