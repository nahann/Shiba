...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    console.log(typ);
    if (
      !["WATCHING", "PLAYING", "STREAMING", "LISTENING", "COMPETING"].includes(
        typ.toUpperCase()
      )
    )
      return message.reply("That's not a valid type ya nerd");
    await client.user.setActivity(status, { type: typ.toUpperCase() });
    message.reply({
      embeds: [
        client.embed(
          {
            description: `Changed status to ${
              types[typ.toUpperCase()]
            } ${status}`,
          },
          message
        ),
