  ...
  run: async (client, message, args) => {
    ...
    fetch(`https://disease.sh/v3/covid-19/countries/${args.join(" ")}`)
      ...
      .then((json) => {
        ...
        const e = client
          .embed({ title: `COVID-19 stats of ${json.country}` }, message)
          .setThumbnail(json.countryInfo.flag)
          .addField(`Cases today`, `\`\`\`${json.todayCases}\`\`\``, true)
          .addField(`Deaths today`, `\`\`\`${json.todayDeaths}\`\`\``, true)
          .addField(`All cases`, `\`\`\`${json.cases}\`\`\``, true)
          .addField(`All deaths`, `\`\`\`${json.deaths}\`\`\``, true)
          .addField(`All recoveries`, `\`\`\`${json.recovered}\`\`\``, true)
          .addField(
